...
void FootModel::Mirror(void)
{
	for(auto & bone : bones){
		...
		bone->matrixinit.UpdateOrientation();
	}
}
...
void FootModel::UpdatePosition(const Shoe &shoe, double offset, double mixing)
{
	...
	std::shared_ptr <Bone> PhalanxI1 = GetBone("PhalanxI1");
	std::shared_ptr <Bone> PhalanxI2 = GetBone("PhalanxI2");
	std::shared_ptr <Bone> PhalanxI3 = GetBone("PhalanxI3");
	std::shared_ptr <Bone> PhalanxI4 = GetBone("PhalanxI4");
	std::shared_ptr <Bone> PhalanxI5 = GetBone("PhalanxI5");
	std::shared_ptr <Bone> Talus = GetBone("Talus");
	std::shared_ptr <Bone> Calcaneus = GetBone("Calcaneus");
	std::shared_ptr <Bone> Cuboideum = GetBone("Cuboideum");
	std::shared_ptr <Bone> Cuneiforme1 = GetBone("Cuneiforme1");
	std::shared_ptr <Bone> Cuneiforme2 = GetBone("Cuneiforme2");
	std::shared_ptr <Bone> Cuneiforme3 = GetBone("Cuneiforme3");
...
void FootModel::CalculateBones(void)
{
	...
	parser.SetVariable("L", L);
	parser.SetVariable("R", R);
	parser.SetVariable("S", S);
	...
	for(auto & bone : bones){
		bb.Insert(
				Vector3(bone->p1.x - bone->r1 - 3 * bone->s1,
						...
						bone->p1.z - bone->r1 - 3 * bone->s1));
		bb.Insert(
				Vector3(bone->p2.x - bone->r2 - 3 * bone->s2,
						bone->p2.y - bone->r2 - 3 * bone->s2,
						bone->p2.z - bone->r2 - 3 * bone->s2));
		bb.Insert(
				Vector3(bone->p1.x + bone->r1 + 3 * bone->s1,
						bone->p1.y + bone->r1 + 3 * bone->s1,
						bone->p1.z + bone->r1 + 0 * bone->s1));
		bb.Insert(
				Vector3(bone->p2.x + bone->r2 + 3 * bone->s2,
						bone->p2.y + bone->r2 + 3 * bone->s2,
						bone->p2.z + bone->r2 + 0 * bone->s2));
	}

	const double cellvol = bb.GetVolume() / 50000.0;
	const double res = cbrt(cellvol);
	skin.SetExtent(bb.GetSizeX(), bb.GetSizeY(), bb.GetSizeZ(), res);
	skin.SetOrigin(Vector3(bb.xmin, bb.ymin, bb.zmin));
