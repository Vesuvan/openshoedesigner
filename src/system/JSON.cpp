...
{
	if(type == Type::Array) return valueArray.size();
	if(type == Type::Object) return valueObject.size();
	return 1;
}

JSON& JSON::operator [](const std::string &key)
{
	if(type == Type::Object) return valueObject[key];
...
{
	if(type == Type::Array) return valueArray.at(index);
	if(type == Type::Object){
...
{
	if(type == Type::Number) return valueNumber;
	if(type == Type::Boolean) return (valueBoolean)? 1 : 0;
	if(type == Type::String) return strtod(valueString.c_str(), NULL);
...
{
	if(type == Type::Boolean) return valueBoolean;
	if(type == Type::Number) return fabs(valueNumber) > 1e-6;
	if(type == Type::Null) return defaultvalue;
...
{
	if(type == Type::String) return valueString;
	if(type == Type::Boolean) return valueBoolean? "true" : "false";
	if(type == Type::Null) return defaultvalue;
...
{
	if(type == Type::Array) return *(valueArray.begin());
	if(type == Type::Object) return valueObject.begin()->second;
	throw(std::logic_error("Not an array or object."));
}
